...
** "message K, texte" ou K dit d'ou vient le son, PAR RAPPORT A LUI. Deux
** drones places differemment recoivent donc des K differents pour le meme
** cri, et c'est ce qui permet a une IA de retrouver un allie.
**
** Vu de l'auditeur, le haut du schema est la direction qu'il regarde.
...
** COMMENT ON TROUVE K
**
** On place d'abord l'emetteur dans le repere de l'auditeur : f cases
** devant lui, r cases a sa droite (le monde etant un tore, on prend le
** chemin le plus court). Puis on raisonne par ANNEAUX CARRES autour de
** l'auditeur : l'emetteur est sur l'anneau n = max(|r|, |f|), qui compte
** 8n cases. On numerote sa position p le long de cet anneau en partant de
** la case droit devant, et c'est p qui donne le secteur.
**
...
**
** Ces regles ont ete relevees sur le serveur de reference, en amenant un
** emetteur exactement sur chaque position voulue autour d'un auditeur
** immobile, sur les anneaux 1 a 6.
*/
...
** demi-decalage qui cree l'asymetrie observee sur la reference.
*/
...
/* Numero de la direction d'ou "to" entend "from". */
static int sound_direction(server_t *srv, client_t *from, client_t *to)
{
    static const int dx[5] = {0, 0, 1, 0, -1};
    static const int dy[5] = {0, -1, 0, 1, 0};
    int ex = shortest_delta(to->x, from->x, srv->width);
    int ey = shortest_delta(to->y, from->y, srv->height);
...
/*
** Crie le message : l'emetteur recoit "ok", tous les AUTRES drones
** recoivent le texte precede de leur direction, et les GUI un pbc.
*/
